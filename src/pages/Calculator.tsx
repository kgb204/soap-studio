...
import { getIngredients, getRecipes, getCurrentPrice } from '../api';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-[#e8d5c4] p-4 shadow-sm">
        ...
        <div className="flex gap-2">
          <select className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#d4956a]" value={selectedRecipeId} onChange={(e) => loadFromRecipe(e.target.value)}>
            <option value="">-- Select a recipe --</option>
            {recipes.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
          </select>
          <button onClick={() => { setLines([{ id: nanoid(), ingredientId: '', amount: 0, unit: 'oz' }]); setSelectedRecipeId(''); setBarsCount(0); }} className="border border-gray-200 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Clear</button>
        </div>
      </div>
            ...
            {lines.map((line) => {
              ...
              return (
                <div key={line.id} className="flex gap-2 items-center">
                  <select className="flex-1 border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-[#d4956a]" value={line.ingredientId} onChange={(e) => updateLine(line.id, { ingredientId: e.target.value })}>
                    <option value="">-- Select --</option>
                    {ingredients.map((i) => <option key={i.id} value={i.id}>{i.name} ({i.unit})</option>)}
                  </select>
                  <input type="number" min="0" step="0.01" className="w-24 border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-[#d4956a]" value={line.amount || ''} onChange={(e) => updateLine(line.id, { amount: parseFloat(e.target.value) || 0 })} placeholder="Amt" />
                  <select className="w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-[#d4956a]" value={line.unit} onChange={(e) => updateLine(line.id, { unit: e.target.value as Unit })}>
        ...
        <div className="space-y-3">
          <div className="bg-[#3d2b1f] text-white rounded-xl p-4 shadow-sm space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[#c4a882]">Ingredients</span><span>{formatCurrency(ingredientCost)}</span></div>
              <div className="flex justify-between"><span className="text-[#c4a882]">Labor</span><span>{formatCurrency(laborCost)}</span></div>
              <div className="flex justify-between"><span className="text-[#c4a882]">Overhead</span><span>{formatCurrency(overheadCost)}</span></div>
          ...
          <div className="bg-white rounded-xl border border-[#e8d5c4] p-4 shadow-sm space-y-3">
            ...
            {(costPerBar != null || pricePerBar != null) && (
              <div className="border-t border-gray-100 pt-3 space-y-2 text-sm">
                {costPerBar != null && <div className="flex justify-between"><span className="text-gray-600">Cost per bar</span><span className="font-semibold text-[#3d2b1f]">{formatCurrency(costPerBar)}</span></div>}
                {markup > 0 && pricePerBar != null && <div className="flex justify-between"><span className="text-gray-600">Suggested price</span><span className="font-bold text-green-700 text-base">{formatCurrency(pricePerBar)}</span></div>}
                {markup > 0 && <div className="flex justify-between"><span className="text-gray-600">Batch revenue</span><span className="font-semibold text-green-700">{formatCurrency(sellingPrice)}</span></div>}
              </div>
            )}
          </div>
        </div>
